...
class EmailVerificationRepository implements EmailVerificationRepositoryInterface
{
    ...
    {
        $stmt = $this->pdo->prepare(
            'SELECT user_id, expires_at, used_at FROM email_verifications WHERE token_hash = :token_hash LIMIT 1'
        );
    ...
    {
        $stmt = $this->pdo->prepare(
            'UPDATE email_verifications SET used_at = NOW() WHERE token_hash = :token_hash'
        );
        $stmt->execute([':token_hash' => $tokenHash]);
    }
